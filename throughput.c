...
static DeviceStats* get_or_create_stats(const char *dbdf) {
    for (int i = 0; i < num_stats; i++) {
        if (strcmp(global_stats[i].dbdf, dbdf) == 0) return &global_stats[i];
    }
    ...
    for (int i = 0; i < total_devices; i++) {
        ...
        if (dir) {
            ...
            while ((entry = readdir(dir)) != NULL) {
                ...
                if (f_rx && f_tx) {
                    ...
                    if (fscanf(f_rx, "%llu", &rx_bytes) == 1 && fscanf(f_tx, "%llu", &tx_bytes) == 1) {
                        ...
                        if (s && s->last_time > 0) {
                            ...
                            if (dt > 0) {
                                all_nodes[i].rx_rate = (float)((rx_bytes - s->last_rx_bytes) / dt / 1024.0 / 1024.0);
                                all_nodes[i].tx_rate = (float)((tx_bytes - s->last_tx_bytes) / dt / 1024.0 / 1024.0);
                            }
                        }
                        if (s) {
                ...
                if (f_rx) fclose(f_rx);
                if (f_tx) fclose(f_tx);
                break; // Just one interface per PCI device usually
            }
            closedir(dir);
        }
    }
                    ...
                    while ((ns_entry = readdir(ns_dir)) != NULL) {
                        if (strstr(ns_entry->d_name, "nvme") && strstr(ns_entry->d_name, "n1")) {
                            ...
                            if (f) {
                                ...
                                // 1: rd_ios, 2: rd_merges, 3: rd_sectors, 4: rd_ticks
                                // 5: wr_ios, 6: wr_merges, 7: wr_sectors, 8: wr_ticks
                                if (fscanf(f, "%llu %llu %llu %llu %llu %llu %llu %llu", 
                                           &rd_ios, &rd_merges, &rd_sectors, &rd_ticks,
                                           &wr_ios, &wr_merges, &wr_sectors, &wr_ticks) == 8) {
